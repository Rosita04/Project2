	...
	var AFS = {
		...
		initAnnouncement: function() {
			...
			if (announcement.length) {
				var cookieName = announcement.attr('data-announcement-cookie');
				var cookie = LBi.getCookie(cookieName);
				if (!cookie) {
					announcement.show();
				}
				announcement.find('.announcement__button-close').bind('click', function(e) {
					e.preventDefault();
					LBi.setCookie(cookieName, 'true', {expires: 0.5});
					announcement.hide();
				});
			}
		},
		...
		initialize: function() {
			 ...
			 *
			 */
			this.triggers = new LBi.Trigger('.trigger:not(.downloadtrigger)');
            this.triggers = new LBi.Trigger('.list_news li');
			this.triggers = new LBi.Trigger('.agenda__list li');
